...
3. [Components](03-components.md): the 73-component catalog and how to import it.
...
7. [Theming](07-theming.md): the token model in depth, and a worked procedure for building your own brand
   preset.
8. [Blocks](08-blocks.md): the seventeen higher-level blocks built on the primitives.
...
- **Theme:** Akira purple by default. Brand presets under `themes/*.css` set the required primary pair
  (`--primary`, `--primary-foreground`) behind a `data-brand` attribute and may add the complete destructive
  pair (`--destructive`, `--destructive-foreground`) in both schemes.
