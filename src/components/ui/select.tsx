...
function SelectTrigger({
    ...
    return (
        <SelectPrimitive.Trigger
            ...
            className={cn(
                "h-12 rounded-2xl bg-zinc-50 px-4 py-2 text-base font-medium text-black shadow-xs focus:border-red-500/50 focus:bg-white focus:ring-red-500/20 data-[placeholder]:text-zinc-400 *:data-[slot=select-value]:gap-2 dark:bg-white/5 dark:text-white dark:focus:bg-white/10 dark:data-[placeholder]:text-white/20 [&_svg:not([class*='size-'])]:size-4 md:text-sm flex w-full items-center justify-between border border-transparent transition-[color,box-shadow] outline-none focus:ring-2 disabled:cursor-not-allowed disabled:opacity-50 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center [&_svg]:pointer-events-none [&_svg]:shrink-0 [&>span]:line-clamp-1",
                className,
            )}
            {...props}
        >
